...
export const reportTemplates: Record<ReportMode, ReportTemplate> = {
  stock: {
    title: "个股分析",
    userLabel: "标的",
    contextLabel: "补充信息",
...
const systemPrompt = `你是阿U智能体，一个面向金融市场研究和投融资材料分析的中文 AI 助手。你要按专业研究报告的方式写作，结构清楚、判断克制、风险意识明确。你不能承诺收益，不能给出保证性投资结论，也不能替代持牌金融顾问。如果缺少事实数据，要明确标注“需要进一步核验”，不要编造具体财务数据、估值倍数或监管事实。每一节都要尽量完整展开，使用中文。`;

export function isReportMode(mode: string): mode is ReportMode {
...
${template.contextLabel}：${context || "用户暂未提供补充材料，请基于通用研究框架展开，并明确提示哪些信息需要进一步核验。"}
